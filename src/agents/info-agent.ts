...
export class AuktrafiInfoAgent extends ASIOneCompatibleAgent {
  ...
  private handlePlatformInfo(): AgentResponse {
    return AgentChatProtocol.createResponse(
      this.agentId,
      `🏠 **Auktrafi** es una plataforma descentralizada para reservas y subastas de bienes raíces.
...
Visita: https://auktrafi.xyz`,
      ...
      'Información general de la plataforma Auktrafi',
      ['¿Cómo funcionan las reservas?', '¿Cómo funcionan las subastas?', 'Ver características']
    );
  }

  private handleReservationInfo(): AgentResponse {
    return AgentChatProtocol.createResponse(
      this.agentId,
      `📋 **Proceso de Reserva en Auktrafi:**
...
- PYUSD para staking
- Conexión a Ethereum o Arbitrum`,
      'success',
      {
        process: 'reservation',
  ...
  private handleAuctionInfo(): AgentResponse {
    return AgentChatProtocol.createResponse(
      this.agentId,
      `🏆 **Subastas en Auktrafi:**
...
- Puja estratégicamente antes del cierre`,
      'success',
      {
        process: 'auction',
  ...
  private handleFeatures(): AgentResponse {
    return AgentChatProtocol.createResponse(
      this.agentId,
      `✨ **Características de Auktrafi:**
...
- Fácil de obtener y usar`,
    ...
    return AgentChatProtocol.createResponse(
      this.agentId,
      `Preguntas frecuentes sobre Auktrafi:
...
Haz una pregunta específica para más detalles.`,
      'success'
    );
  }
  ...
  private handleHelp(): AgentResponse {
    return AgentChatProtocol.createResponse(
      this.agentId,
      `👋 ¡Hola! Soy el Agente Informativo de Auktrafi.
...
¡Pregúntame lo que necesites! 🏠`,
      'success',
      undefined,
      'Información de ayuda del agente',
      ['Ver información de la plataforma', 'Cómo reservar', 'Cómo funcionan las subastas']
    );
  }

  private handleGeneralInfo(query: string): AgentResponse {
    return AgentChatProtocol.createResponse(
      this.agentId,
      `Información sobre Auktrafi:
...
O di "ayuda" para ver todas las opciones disponibles.`,
      'success',
      { query },
      'Respuesta general a consulta'
    );
  }

  private parseIntent(query: string): { type: string; confidence: number } {
    const lowerQuery = query.toLowerCase();
